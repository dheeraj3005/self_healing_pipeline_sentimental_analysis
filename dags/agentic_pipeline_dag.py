...
#Prevent SIGKILL due to thread overuse
...
class Config:
    BASE_DIR = os.getenv('PIPELINE_BASE_DIR', '/Users/dheerajlokesh/Documents/Dheeraj files/projects/self_healing_pipeline_sentiment_analysis')
...
default_args = {
    ...
    'execution_timeout': timedelta(minutes=30) #You can include start_date here if needed and schedule_interval below and other default args.
}
...
def _load_ollama_model(model_name:str):
    ...
    try:
        ...
        try:                                            #if model is not found, it tries to pull from remote repo
    ...
    test_response = client.chat(model = model_name,
            messages = [{"role": "user", 
             "content": "Classify the sentiment of the review: 'I love this product!' as Positive, Negative, or Neutral."}],
                                )
    test_results = test_response['message']['content']
    logger.info(f"Test response from model {model_name}: {test_results}")
...
def _load_from_file(param: dict, batch_size: int, offset: int):
    input_file = param.get('input_file', Config.INPUT_FILE)
...
@dag(
    dag_id='self_healing_agentic_pipeline_dag',
    default_args=default_args,
    description='An agentic pipeline DAG for sentiment analysis using Ollama',
    start_date=datetime(2026, 1, 4),
    catchup=False,
    tags=['sentiment_analysis', 'ollama', 'nlp','agentic_pipeline'],
    params={
        'input_file': Param(
...
def self_healing_pipeline():
    ...
    def diagnose_and_heal_batch(reviews:list[dict]):
        ...
        healed_count = sum(1 for r in healed_reviews if r.get('healed', True))
        logger.info(f"Healed {healed_count} out of {len(reviews)} reviews.")
